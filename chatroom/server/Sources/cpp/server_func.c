...
static int user_id_create(char *id)
{
    ...
    {
        mysqlite3_getdata("select lid from logout where seq = 2;", &id_tmp, LOGOUT);
    }
...
void id_init(void)
{
    ...
    mysqlite3_open();
    //初始化id_counter
    mysqlite3_getdata("select lid from logout where seq = 0;", &id_counter.lid_num, LOGOUT);
    mysqlite3_getdata("select lid from logout where seq = 1;", &id_counter.uid_max, LOGOUT);
    printf("lid_num = %d\n", id_counter.lid_num);
    printf("uid_max = %d\n", id_counter.uid_max);
}
...
static void reg_handler(msg_sql_t *reg_msg)
{
    ...
    //执行sql语句,成功则代表注册成功
    if(SQL_SUCCESS == mysqlite3_exec(reg_msg->sql_buf))      //成功
    {
        //修改id_counter
        ...
        else
        {
            ...
            mysqlite3_exec(sql_buf);
            mysqlite3_exec("update logout set seq = seq - 1 where seq > 1;");
            id_counter.lid_num--;
        }
        //通知客户端注册成功
...
static void login_handler(msg_sql_t *login_msg)
{
    ...
    //查询user表判断账户密码是否正确
    if(SQL_SUCCESS == mysqlite3_getdata(login_msg->sql_buf, data, ONLINE) && 10 == strlen(my_info.myid))
    {
        //检测重复登陆，不能重复登录
...
static void find_psd_handler(msg_sql_t *find_psd_msg)
{
    ...
    //用于保存查询到的的密码
    char password[21] = {0};
    if(SQL_SUCCESS == mysqlite3_getdata(find_psd_msg->sql_buf, password, FIND) && 0 != strlen(password))
    {
...
static void leave_word_handler(msg_chat_t *chat_msg)
{
    ...
    if(SQL_FAILURE == mysqlite3_exec(sql_buf))
    {
...
static void look_word_handler(msg_sql_t *look_msg)
{
    ...
    char **result;
    int row, col;
    int ret = mysqlite3_get_table(look_msg->sql_buf, &result, &row, &col);
    if(SQL_FAILURE == ret)
    {
        ser_msg.ctl.cmd = OPT_ERR;
...
static void update_handler(msg_sql_t *update_msg)
{
    ...
    ser_msg.ctl.cfd = -1;
    ser_msg.ctl.msg_size = sizeof(ser_msg_t);
    if(SQL_FAILURE == mysqlite3_exec(update_msg->sql_buf))
    {
        ser_msg.ctl.cmd = OPT_ERR;
    ...
    else
    {
        ...
        mysqlite3_getdata(sql_buf, data, ONLINE);
        p->vip = atoi(vip);
...
static void check_all_handler(msg_sql_t *admin_msg)
{
    ...
    ser_msg_t ser_msg;
    ser_msg.ctl.msg_size = sizeof(ser_msg_t);
    if(SQL_FAILURE == mysqlite3_get_table(admin_msg->sql_buf, &result, &row, &col))
    {
        ser_msg.ctl.cmd = -1;
        strcpy(ser_msg.reply, "查看失败!");   
...
static void talk_handler(msg_who_t *ban_msg)
{
    ...
    //修改数据库ban
    if(SQL_FAILURE == mysqlite3_exec(ban_msg->sql_buf))
    {
        ser_msg.ctl.cmd = -1;
